...
# Registry tool name -> conda package base, only where they differ. Used both to pick a config's
# core-env packages and by the wiring audit to prove every tool is provisioned by SOME env.
TOOL_PACKAGE_ALIASES = {
    "mapdamage": "mapdamage2",
    "blastn": "blast",
    "metaspades": "spades",
    "eggnog": "eggnog-mapper",
    "amrfinderplus": "ncbi-amrfinderplus",
}
_TOOL_PACKAGE_ALIASES = TOOL_PACKAGE_ALIASES  # backward-compatible alias


def conda_package(tool: str) -> str:
    """Conda package base for a registry tool name (identity unless it differs)."""
    return TOOL_PACKAGE_ALIASES.get(tool, tool)


def _env_yaml_packages(env_yml_path: Optional[str]) -> Dict[str, str]:
    """Parse the bundled environment.yml into {package_base: full_pinned_spec}."""
    out: Dict[str, str] = {}
    if not env_yml_path or not os.path.isfile(env_yml_path):
        return out
    with open(env_yml_path) as fh:
        data = yaml.safe_load(fh) or {}
    for dep in data.get("dependencies", []) or []:
        if isinstance(dep, str):
            base = re.split(r"[ <>=]", dep, 1)[0].strip()
            if base:
                out[base] = dep.strip()
    return out


def active_core_packages(cfg: Dict[str, Any], env_yml_path: Optional[str]) -> List[str]:
    """Pinned conda specs for THIS config's active tools that live in the shared core env — the
    tools the run actually calls, not the full default stack. Heavy/domain tools are excluded on
    purpose: their rule ships its own workflow/envs/*.yaml and `metagx run --use-conda` provisions
    them per-rule, so they don't belong in the user's base env."""
    envmap = _env_yaml_packages(env_yml_path)
    picked: List[str] = []
    for tool in sorted(set(active_tools(cfg))):
        spec = envmap.get(_TOOL_PACKAGE_ALIASES.get(tool, tool))
        if spec and spec not in picked:
            picked.append(spec)
    return picked


def config_env_yaml(cfg: Dict[str, Any], env_yml_path: Optional[str],
                    name: Optional[str] = None) -> str:
    """A minimal conda environment.yml for exactly the tools THIS config needs — generated after
    the interview, so a new user builds a small env instead of the full default stack."""
    envmap = _env_yaml_packages(env_yml_path)
    python_spec = envmap.get("python", "python >=3.10")
    name = name or f"metagx-{cfg.get('project', 'run')}"
    lines = [
        "# Minimal conda env for THIS analysis (generated by `metagx env-file --config`).",
        "# Only the tools this config's modules call — not the full default stack. Heavy/domain",
        "# tools (if any) are provisioned per-rule by `metagx run --use-conda`.",
        f"name: {name}",
        "channels: [conda-forge, bioconda]",
        "dependencies:",
        f"  - {python_spec}",
    ]
    lines += [f"  - {p}" for p in active_core_packages(cfg, env_yml_path)]
    return "\n".join(lines) + "\n"
